...
#
#   gunyah_host_share  SHARE_BLOB / GuestAccept   -> 6.6, 6.12   (upstream gunyah_*)
...
#
# NOTE (kvcalloc / 6.1): gunyah_kvcalloc #include's the private gunyah driver header
# vm_mgr.h, so its build adds -I<kdir>/drivers/virt/gunyah and needs a DDK image whose
# kdir contains drivers/virt/gunyah/. gunyah_host_share needs no driver source (it reads
# the private structs by BTF-verified offset). The 6.1 host-share port is still pending
# (downstream gh_rm_* naming); until it lands, 6.1 ships kvcalloc only.
set -euo pipefail
cd "$(dirname "$0")"
OUT=dist

# build_mod <kmi-tag> <ddk-image> <src-dir> <src-file> <mod-name> <need-gunyah-hdr:0|1>
build_mod() {
  local tag="$1" img="$2" srcdir="$3" srcfile="$4" mod="$5" needhdr="${6:-0}"
  echo ">>> building ${mod}.ko for ${tag}"
  mkdir -p "${OUT}/${tag}"
  docker run --rm \
    -v "${PWD}/${srcdir}:/src:ro" \
    -v "${PWD}/${OUT}/${tag}:/out" \
    -e MOD="${mod}" -e SRCFILE="${srcfile}" -e NEEDHDR="${needhdr}" \
    "${img}" bash -lc '
      set -e
      ...
      printf "obj-m += %s.o\n" "$MOD" > "$B/Makefile"
      [ "$NEEDHDR" = 1 ] && printf "ccflags-y += -I%s/drivers/virt/gunyah\n" "$K" >> "$B/Makefile"
